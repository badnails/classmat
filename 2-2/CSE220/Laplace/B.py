...
class SmartIrrigation:
    ...
    def quantize(self, u):
        return self.delta * np.round(u / self.delta)
    
    def shift(self, u):
        return np.interp(self.t - self.td, self.t, u, left = 0, right = 0)
    
    def u_error_C(self, u):
        return self.shift(self.quantize(u))
    ...
    def MAE(self, h, he):
        return 1/self.t_max * np.trapezoid(np.abs(h - he), self.t)
    
    def PAE(self, h, he):
        return np.max(np.abs(h - he))

    def compute_metrics(self, h, he):
...
for idx, (name, u) in enumerate(inputs.items()):
    ...
    #Error
    u_e = system.u_error_C(u)
    U_s_vals_e = np.array([system.laplace_transform(u_e, s) for s in s_list])
    H_s_vals_e = system.H_s(s_list, U_s_vals_e)
    h_laplace_e = system.inverse_laplace(s_list, H_s_vals_e)

    print(f"\n  ► {name}")
    metrics = system.compute_metrics(h_laplace, h_laplace_e)
    ...
    axes[0].set_title("Laplace Transform Simulation (Clean)", fontweight='bold')
    ...
    # Euler subplot
    axes[1].plot(system.t, u_e, 'b--', lw=1.8, label="Input u(t)")
    axes[1].plot(system.t, h_laplace_e, color='tomato', lw=2.2, label="Output h(t)")
    axes[1].set_title("Laplace Method Simulation (Error)", fontweight='bold')
