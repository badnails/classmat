...
def plot_spectrum(X, prefix=""):
    ...
    plt.figure()
    plt.stem(k, mag, use_line_collection=True)
    ...
    plt.figure()
    plt.stem(k, ph, use_line_collection=True)
...
def main():
    ...
    for name, x in signals.items():
        ...
        plt.stem(n, np.real(x_hat), use_line_collection=True, markerfmt="C1o", linefmt="C1-")
    ...
    plt.figure()
    n = np.arange(4)
    plt.stem(n, np.real(y_time), use_line_collection=True)
    plt.stem(n, np.real(y_freq), use_line_collection=True, markerfmt="C1o", linefmt="C1-")
