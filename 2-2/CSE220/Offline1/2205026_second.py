...
def read_input_from_file(filename):
    ...
    start_idx, end_idx = map(int, lines[0].strip().split())
    
    values = np.array(list(map(float, lines[1].strip().split())))
    
    INF = max(abs(start_idx), abs(end_idx)) + 10
...
if __name__ =="__main__":
    input_signal = read_input_from_file('input_signal.txt')
    
    input_signal.plot("Noisy Input Signal")
    
    INF = 34
    h = Signal(INF)
    h.set_value_at_time(-2, 1/5)
    h.set_value_at_time(-1, 1/5)
    h.set_value_at_time(0, 1/5)
    h.set_value_at_time(1, 1/5)
    h.set_value_at_time(2, 1/5)
    ...
    output_signal.plot("Smoothed Output Signal (5-point Moving Average)")